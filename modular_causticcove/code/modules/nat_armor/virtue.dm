/datum/virtue/combat/natarmor
	name = "Natural Armor"
	desc = "My hide is thick and resilient, enhancing my natural defense whilst making making armor meerly vanity on my body. \
	\n It will regenerate so long as I keep it fed...\
	\n NOTE: OVERRIDES ANY WORN ARMOR."
	added_traits = list()
	custom_text = "My hide is thick and resilient, enhancing my natural defense whilst making armor meerly vanity on my body. \
	\n It will regenerate so long as I keep it fed...\
	\n NOTE: OVERRIDES ANY WORN ARMOR ON THE BODY."

/datum/virtue/combat/natarmor/apply_to_human(mob/living/carbon/human/recipient)
	recipient.skin_armor = new /obj/item/clothing/suit/roguetown/armor/skin_armor/natural_armor(recipient)
