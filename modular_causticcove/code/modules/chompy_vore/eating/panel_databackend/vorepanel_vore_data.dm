	...
	if(owner.vore_selected)
		...
		if(active_vore_tab == VISUALS_TAB)
			...
			if(selected.colorization_enabled)
				belly_fullscreens = cached_icon_states('icons/mob/vore_fullscreens/screen_full_vore_list.dmi') //Makes any icons inside of here selectable.
			else
				belly_fullscreens = cached_icon_states('icons/mob/vore_fullscreens/screen_full_vore.dmi') //Non colorable
