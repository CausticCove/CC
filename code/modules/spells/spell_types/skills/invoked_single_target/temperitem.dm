...
/obj/effect/proc_holder/spell/invoked/temperitem
	...
	You can sacrifice a gold ore/ingot, and a steel ingot, to make the effects permanent. \n\ "
	releasedrain = 50
	chargedrain = 0
	chargetime = 0
...
/obj/effect/proc_holder/spell/invoked/temperitem/cast(list/targets, mob/user = usr)
	...
	if(istype(target, /obj/item/clothing/))
		...
		if((!(clothingenchant.anvilrepair)||isnull(clothingenchant.anvilrepair)))
			to_chat(user, span_warning("You can only temper medium, or heavy armor pieces."))
			return
